use super::index::{Block, Index, BLOCK_PITCH};
use super::insbin::InsTracker;
use super::op::{op_len, Cigar, CigarOp, CompressMark, IntoOpsIterator};
use super::utils::{atoi_unchecked, transcode_base_unchecked, SlicePrecursor, Writer};
use std::io::Write;
...
impl<'a, 'b> Index<'a> {
    ...
    fn compose_box(buf: Vec<u8>, precursor: Precursor) -> Box<Index<'a>> {
        ...
        let mut udon = unsafe {
            ...
            let mut udon = Box::<Index<'a>>::from_raw(transmute::<*mut u8, *mut Index<'a>>(base));
...
impl<'a> Precursor {
    fn build_core(
        ...
        let (cigar, clip) = match strip_clips(cigar) {
            None => {
                return (buf, None);
            } /* just return buffer (nothing happened) */
...
impl<'a> Builder<'a> {
    ...
    */
    ...
    fn eat_match(&mut self, xrem: usize, last_op: u32) -> Option<usize> {
        ...
        let mut crem = if is_valid {
            self.eat_cigar().unwrap().1
        } else {
            0
        } + last_op as usize;
    ...
    fn save_ins(&mut self, qofs: usize, qlen: usize) {
        if qlen == 0 {
            return;
        } /* must be corrupted cigar, but safe to ignore */
