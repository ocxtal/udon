...
use super::op::{op_is_cont, op_len, op_marker, CompressMark};
...
impl<'a, 'b> Index<'a> {
    ...
    unsafe fn decode_core_block(dst: &mut [u8], op: u8, ins: u32) -> (usize, u32) {
        ...
        let scatter_mask =
            _mm_load_si128(Self::SCATTER_MASK.as_ref() as *const [u8; 16] as *const __m128i);
        let is_del_thresh =
            _mm_load_si128(Self::IS_DEL_THRESH.as_ref() as *const [u8; 16] as *const __m128i);
        ...
        let marker = if op_marker(op) == CompressMark::Ins as u32 {
            ins
        } else {
            op_marker(op)
        };
        ...
        */
        let next_ins = if op_is_cont(op) {
            0
        } else {
            UdonOp::Ins as u32
        }; /* next ins will be masked if 0x1f */
    ...
    fn decode_core(&self, dst: &mut Vec<u8>, ref_span: &Range<usize>) -> Option<usize> {
        ...
        */
        ...
        /* tail */
        {
            ...
            while ofs < end {
                let op = ops.next()?;
                let (block_len, next_ins) =
                    unsafe { Self::decode_core_block(&mut buf[ofs..], *op, ins) };
