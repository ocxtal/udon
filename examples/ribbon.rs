...
impl Pileup {
    ...
    fn push(&mut self, ribbon: &[[[u8; 4]; 2]], horizontal_offset: usize) -> Option<()> {
        ...
        let right_blank = self
            .params
            .window
            .x
            .saturating_sub(ribbon.len() + horizontal_offset);
        ...
        for i in 0..self.params.ribbon.height {
            let idx = if i == self.params.ribbon.height / 2 {
                1
            } else {
                0
            };
    ...
    fn render(&self, filename: &PathBuf) -> Option<()> {
        ...
        encoder
            .encode(
                &self.buf[..3 * self.total_width() * self.total_height()],
    ...
    fn append_border_row(&mut self) -> Option<usize> {
        ...
        self.fill(
            &border,
            self.params.window.x + 2 * self.params.border.thickness,
        );
        self.fill(&background, self.params.margin.x);
        Some(self.total_width())
    }
...
fn main() {
    ...
    let mut reader = BamReader::from_path(&filename, 2).expect(
        format!(
            "Failed to open file `{:?}'. Please check the file exists.",
            &filename
        )
        .as_str(),
    );
    ...
    let window = Range::<usize> {
        start: opt.start,
            ...
            .end
            .min(reader.header().reference_len(id).unwrap() as usize),
    };
    ...
    let mut pileup = Pileup::new(&PileupParams {
        window: Dimension {
            x: opt.width,
            y: opt.height,
        },
    ...
    while let Ok(true) = reader.read_into(&mut record) {
        ...
        if 3 * window_range.len() < window.len() {
            continue;
        }
        ...
        /* slice ribbon scaled */
        let mut ribbon = udon
            .decode_scaled(&udon_range, offset_in_pixel, &scaler)
            .expect(&format!(
                "Failed to decode udon ribbon. Would be a bug. ({:?})",
                &record.name()
            ));

        /* put forward / reverse color then do gamma correction */
        ribbon
            .append_on_basecolor(&base_color[record.flag().is_reverse_strand() as usize])
            .correct_gamma();
    ...
    pileup
        .render(&opt.output)
        .expect(format!("failed to dump image to `{:?}'", &opt.output).as_str());
}
